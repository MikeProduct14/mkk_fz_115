...
- **Стандартизация адресов через Dadata** — в формах новой/редактирования анкеты клиента
  поля «Адрес регистрации» и «Адрес проживания» сейчас свободный ввод (Textarea).
  Нужно добавить автодополнение через Dadata Suggestions API, чтобы адреса сохранялись
  в стандартизированном виде. Потребует: новый Route Handler `/api/dadata-address`,
  debounced Combobox-компонент с подсказками, замену Textarea на этот компонент.
